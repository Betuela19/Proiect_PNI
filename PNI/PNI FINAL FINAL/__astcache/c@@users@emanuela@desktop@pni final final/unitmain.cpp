...
void __fastcall TMainForm::DrawSourceHistogram()
{
  ...
  chartSource->SaveToBitmapFile("ChartSource.bmp");
...
chartDest->SaveToBitmapFile("ChartDestination.bmp");
}
...
void __fastcall TMainForm::Detectiamuchiilor1Click(TObject *Sender)
{
         Graphics::TBitmap *source = new Graphics::TBitmap;
		 source->Assign( imgSource->Picture->Bitmap );
		 source->PixelFormat = imgSource->Picture->Bitmap->PixelFormat;
...
void __fastcall TMainForm::InformatiiAutor1Click(TObject *Sender)
{
    ShowMessage("Coroama Betuela\nGrupa: 3131B\nProiect disciplina PNI");
}
//---------------------------------------------------------------------------
